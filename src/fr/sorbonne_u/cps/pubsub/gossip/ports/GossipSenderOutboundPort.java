...
/**
 * Port sortant utilisé par un courtier pour pousser des messages gossip vers un voisin de la
 * fédération.
 *
 * <p>Un broker maintient un index inverse {@code sendersByNeighbour} (URI de réflexion du voisin
 * -&gt; instance de ce port) afin de sélectionner précisément les destinataires de la re-diffusion
 * et d'appliquer la skip-echo (cf. {@code docs/GOSSIP.md} §4).
 *
 * <p><strong>Politique d'exceptions</strong>
 *
 * <ul>
 *   <li>Toute exception remontée par le connecteur est ré-empaquetée en {@link RemoteException}
 *       pour respecter le contrat RMI de {@link GossipSenderCI#send(GossipMessageI[])}.
 *   <li>Aucune exception métier n'est exposée : un envoi gossip est best-effort, la dédup atomique
 *       côté voisin garantit la convergence.
 * </ul>
 *
...
public class GossipSenderOutboundPort extends AbstractOutboundPort implements GossipSenderCI {
	 ...
	 * @throws Exception déclaré par le contrat {@link GossipSenderCI#send(GossipMessageI[])}.
	 */
	@Override
