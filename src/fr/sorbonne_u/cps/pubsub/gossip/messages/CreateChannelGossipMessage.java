...
/**
 * Message gossip propageant la <strong>création d'un canal privilégié</strong>
 ...
 * {@code createChannel(channel, owner, ownerClass, authorisedUsers)} : le voisin
 * matérialise localement le canal privilégié pour pouvoir y accepter
 * publications et souscriptions. Voir le branchement de ce type dans
 * {@code Broker#update(...)}.
 * </p>
 *
 * <p><strong>Garanties anti-loop / skip-echo</strong></p>
 * <ul>
 *   <li>Implémente {@link EmitterAwareGossipMessageI} : le broker récepteur
 *   utilise {@link #getEmitterURI()} pour ne <em>pas</em> renvoyer le message
 *   au voisin qui vient de nous le transmettre (skip-echo,
 *   cf. {@code docs/GOSSIP.md} §4).</li>
 *   <li>{@link #gossipMessageURI()} est un identifiant unique et immuable
 *   généré par l'émetteur originel ; il sert de clef à la dédup atomique
 *   {@code processedGossipURIs.putIfAbsent(...)} (cf. {@code docs/GOSSIP.md} §3).</li>
 *   <li>{@link #copyWithNewEmitterURI(String)} ne modifie que l'émetteur,
 *   l'URI gossip reste identique : la dédup couvre l'intégralité du chemin.</li>
 * </ul>
 *
...
public class CreateChannelGossipMessage implements EmitterAwareGossipMessageI {
    ...
    /** Regex (au sens {@link java.util.regex.Pattern}) listant les URIs de
     *  réception autorisés à publier/souscrire ; peut être {@code null}. */
    ...
    /**
     * Retourne une copie identique au présent message, avec uniquement
     * l'émetteur ré-écrit. L'URI gossip est conservé, ce qui permet à la
     * dédup ({@code processedGossipURIs.putIfAbsent}) d'attraper l'écho
     * sur n'importe quel chemin du graphe.
     *
