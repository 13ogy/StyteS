...
/**
 * Message gossip propageant l'<strong>enregistrement d'un client</strong> à tous les courtiers
 * voisins de la fédération.
 *
 * <p><strong>Mutation broker répliquée (CDC §3.5)</strong>
 *
 * <p>Équivaut, côté broker récepteur, à un appel local {@code register(clientReceptionPortURI,
 * registrationClass)} : chaque voisin mémorise localement l'existence du client et sa classe de
 * service. Cela permet aux brokers distants de prendre des décisions d'autorisation et de quota
 * cohérentes (cf. {@code Broker#update(...)} sur ce type).
 *
