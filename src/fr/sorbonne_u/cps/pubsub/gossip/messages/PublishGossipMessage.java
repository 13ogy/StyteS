...
/**
 * Message gossip propageant la <strong>publication d'un message</strong> sur un canal à tous les
 * courtiers voisins de la fédération.
 *
 * <p><strong>Mutation broker répliquée (CDC §3.4)</strong>
 *
 * <p>Équivaut, côté broker récepteur, à un appel local {@code publish(channel, pubMessage,
 * publisherReceptionPortURI)} : le voisin intègre le message dans son pipeline de livraison locale
 * (matching de filtres + push aux abonnés concernés).
 *
 ...
 *   <li>{@link #gossipMessageURI()} unique et immuable, clef de dédup (cf. {@code docs/GOSSIP.md}
 *       §3) — garantit qu'un même message publié ne sera pas livré deux fois aux abonnés locaux
 *       d'un broker maillé.
 *   <li>{@link #copyWithNewEmitterURI(String)} ne change que l'émetteur.
 * </ul>
 *
...
public class PublishGossipMessage extends AbstractGossipMessage {
	 ...
	 */
	public String getPublisherReceptionPortURI() {
		return this.publisherReceptionPortURI;
	}
