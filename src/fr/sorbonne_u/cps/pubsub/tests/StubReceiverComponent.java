...
/**
 * Composant stub minimal (TEST-ONLY) qui publie un port d'entrée {@link ReceivingCI} pour permettre
 * au {@link fr.sorbonne_u.cps.pubsub.base.components.Broker} de se connecter à lui pendant les
 * tests d'enregistrement.
 *
 * <p>Ce composant n'a aucune logique métier au-delà de la création/publication du port. Il est
 * utilisé dans {@link BrokerRegistrationTest} à la place de {@link
 * fr.sorbonne_u.cps.pubsub.base.components.PluginClient} pour éviter une pré-condition BCM qui se
 * déclenche sous {@code -ea} lorsqu'un plugin tente d'ajouter une interface offerte déjà déclarée
 * par {@code @OfferedInterfaces}.
 *
 * <p><strong>Cette classe n'est PAS du code de production</strong> ; elle vit uniquement dans le
 * package {@code tests/}.
 *
