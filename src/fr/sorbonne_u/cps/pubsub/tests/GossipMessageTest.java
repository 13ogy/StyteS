...
/**
 * Unit tests for the gossip message hierarchy ({@link AbstractGossipMessage} and its concrete
 * subclasses).
 *
 * <p>These tests validate the three contracts every gossip message must satisfy, regardless of
 * payload :
 *
 * <ol>
 *   <li>Constructor preserves every payload field unchanged (no silent munging).
 *   <li>{@link GossipMessageI#copyWithNewEmitterURI(String)} returns an instance of the <em>same
 *       concrete subclass</em>, with the new emitter URI, and the original {@code gossipMessageURI}
 *       + {@code timestamp} preserved (key invariants for skip-echo + atomic dedup, cf. {@code
 *       docs/GOSSIP.md} §3 §4).
 *   <li>Each concrete subclass implements {@link EmitterAwareGossipMessageI}, inherited
 *       transitively via {@link AbstractGossipMessage}, so the broker can rely on it defensively
 *       (no {@code instanceof} chain).
 * </ol>
 *
 * <p>The {@code gossipMessageURI} of every distinct in-flight message is also asserted unique
 * within the test suite — this is the property the broker's atomic dedup depends on.
 *
...
public class GossipMessageTest {
	...
	public void registerGossip_constructorAndCopy() {
		...
		assertEquals(
				"payload field 2 preserved", RegistrationClass.STANDARD, cm.getRegistrationClass());
	}
	...
	public void unregisterGossip_constructorAndCopy() {
		...
		final UnregisterGossipMessage m =
				new UnregisterGossipMessage(uri, ts, INITIAL_EMITTER, "client-1");
	...
	public void everyConcreteMessage_isAbstractGossipMessage_andEmitterAware() {
		final Instant ts = Instant.now();
		final GossipMessageI[] all =
				new GossipMessageI[] {
					new RegisterGossipMessage(
							fresh("a"), ts, INITIAL_EMITTER, "c", RegistrationClass.FREE),
					new UnregisterGossipMessage(fresh("b"), ts, INITIAL_EMITTER, "c"),
					new PublishGossipMessage(
							fresh("c"), ts, INITIAL_EMITTER, new Message("p"), "ch", "pub"),
					new CreateChannelGossipMessage(
							fresh("d"),
							ts,
							INITIAL_EMITTER,
					...
					new ModifyServiceClassGossipMessage(
							fresh("f"), ts, INITIAL_EMITTER, "c", RegistrationClass.PREMIUM),
					new ModifyAuthorisedUsersGossipMessage(
							fresh("g"),
							ts,
							INITIAL_EMITTER,
		...
		for (GossipMessageI m : all) {
			assertTrue(
					"must be an AbstractGossipMessage: " + m.getClass(),
					m instanceof AbstractGossipMessage);
			assertTrue(
					"must be an EmitterAwareGossipMessageI: " + m.getClass(),
					m instanceof EmitterAwareGossipMessageI);
			assertEquals(
					"getEmitterURI matches via the EmitterAware sub-interface",
					INITIAL_EMITTER,
	...
	public void gossipMessageURIs_areUniquePerInstance() {
		...
		for (int i = 0; i < 100; i++) {
			...
			assertTrue(
					"gossipMessageURI must be unique across instances: " + m.gossipMessageURI(),
	...
	public void copyWithNewEmitterURI_returnsDifferentInstance() {
		...
		final GossipMessageI copy = m.copyWithNewEmitterURI(NEW_EMITTER);
		assertNotEquals(
				"copy must be a fresh object (skip-echo correctness)",
				System.identityHashCode(m),
				System.identityHashCode(copy));
		assertNotEquals(
				"emitterURI was rewritten",
				m.getEmitterURI(),
	...
	public void copyWithNewEmitterURI_acceptsNullEmitter() {
		...
		assertNull(
				"copy with null emitter is null",
				((EmitterAwareGossipMessageI) copy).getEmitterURI());
	}
}
