...
/**
 * Unit tests for the {@link GossipMessageVisitor} double-dispatch mechanism (Visitor design pattern
 * adopted in commit {@code 17435bb} to replace the {@code instanceof} chain in {@code
 * Broker.update(...)} — pour bénéficier du dispatch dynamique sans {@code instanceof}).
 *
 * <p>Each test posts one concrete gossip message to a recording visitor and asserts that the
 * dispatch resolves to the type-correct {@code visit(...)} overload, with the exact same instance
 * passed through (no defensive copy).
 *
 * <p>This is the property {@code Broker.update(...)} relies on every time it does {@code
 * ((AbstractGossipMessage) msg).accept(handler)} — without it, mutations would dispatch to the
 * wrong handler method and the federation would silently corrupt local broker state.
 *
...
public class GossipMessageVisitorTest {
	...
	public void register_dispatchesToRegisterVisit() {
		...
		assertEquals(
				"dispatched to visit(RegisterGossipMessage)", "register", visitor.lastVisitKind);
		assertSame("same message instance forwarded", m, visitor.lastVisited);
	}

	@Test
	public void unregister_dispatchesToUnregisterVisit() {
		final UnregisterGossipMessage m =
				new UnregisterGossipMessage("g-2", ts, "broker-A", "client-1");
		((AbstractGossipMessage) m).accept(visitor);
	...
	public void publish_dispatchesToPublishVisit() {
		final PublishGossipMessage m =
				new PublishGossipMessage(
						"g-3", ts, "broker-A", new Message("hello"), "channel0", "publisher-port");
		((AbstractGossipMessage) m).accept(visitor);
		assertEquals(1, visitor.totalCalls());
	...
	public void createChannel_dispatchesToCreateChannelVisit() {
		final CreateChannelGossipMessage m =
	...
	public void destroyChannel_dispatchesToDestroyChannelVisit() {
		final DestroyChannelGossipMessage m =
				new DestroyChannelGossipMessage("g-5", ts, "broker-A", "priv-1", "owner");
		((AbstractGossipMessage) m).accept(visitor);
		assertEquals(1, visitor.totalCalls());
	...
	public void modifyServiceClass_dispatchesToModifyServiceClassVisit() {
		final ModifyServiceClassGossipMessage m =
	...
	public void modifyAuthorisedUsers_dispatchesToModifyAuthorisedUsersVisit() {
		final ModifyAuthorisedUsersGossipMessage m =
	...
	public void successiveVisits_dispatchIndependently() {
		final RegisterGossipMessage m1 =
				new RegisterGossipMessage(
						"g-A", ts, "broker-A", "client-1", RegistrationClass.FREE);
		final UnregisterGossipMessage m2 =
				new UnregisterGossipMessage("g-B", ts, "broker-A", "client-1");
	...
	/**
	 * Visitor that records the kind of the last call + the argument instance — lets each test
	 * assert dispatch correctness without mocking-framework dependency.
	 */
	private static final class RecordingVisitor implements GossipMessageVisitor {
		...
		@Override
		public void visit(RegisterGossipMessage m) {
			this.lastVisitKind = "register";
			this.lastVisited = m;
			this.callCount++;
		}

		@Override
		public void visit(PublishGossipMessage m) {
			this.lastVisitKind = "publish";
			this.lastVisited = m;
			this.callCount++;
		}

		@Override
		public void visit(CreateChannelGossipMessage m) {
			this.lastVisitKind = "createChannel";
			this.lastVisited = m;
			this.callCount++;
		}

		@Override
		public void visit(DestroyChannelGossipMessage m) {
			this.lastVisitKind = "destroyChannel";
			this.lastVisited = m;
			this.callCount++;
		}

		@Override
		public void visit(ModifyServiceClassGossipMessage m) {
			this.lastVisitKind = "modifyServiceClass";
			this.lastVisited = m;
			this.callCount++;
		}

		@Override
		public void visit(ModifyAuthorisedUsersGossipMessage m) {
			this.lastVisitKind = "modifyAuthorisedUsers";
			this.lastVisited = m;
			this.callCount++;
		}

		@Override
		public void visit(UnregisterGossipMessage m) {
			this.lastVisitKind = "unregister";
			this.lastVisited = m;
			this.callCount++;
		}
	}
