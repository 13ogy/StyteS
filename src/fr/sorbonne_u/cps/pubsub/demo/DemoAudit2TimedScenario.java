...
/**
 * Scénario d'intégration temporisé pour l'audit 2 (CDC §4.4) en utilisant l'outil de tests BCM4Java
 * (Annexe B : {@link TestScenario} + {@link ClocksServer}).
 *
 * <p>Objectifs démontrés :
 *
 * <ul>
 *   <li>Appels asynchrones pour la publication (pipeline asynchrone dans le broker).
 *   <li>Parallélisme explicite via trois pools de threads (réception/propagation/livraison).
 *   <li>Concurrence maîtrisée (sections critiques dans le broker).
 *   <li>Modes de réception avancés côté client : {@code getNextMessage} et {@code
 *       waitForNextMessage(Duration)}.
 * </ul>
 *
...
public class DemoAudit2TimedScenario extends AbstractCVM {
							...
							owner -> {
								try {
									System.out.println(
											"[Audit2Timed][STEP@"
													+ tRegister
													+ "] B : enregistrement PREMIUM");
									((PluginClient) owner).register(RegistrationClass.PREMIUM);
									owner.traceMessage("[Audit2Timed] B enregistré (PREMIUM)\n");
								} catch (Exception e) {
									owner.traceMessage(
											"[Audit2Timed] B : échec enregistrement : " + e + "\n");
								}
							}),
					new TestStep(
							CLOCK_URI,
								...
								try {
									System.out.println(
											"[Audit2Timed][STEP@"
													...
													+ CHANNEL);
									((PluginClient) owner)
											.subscribe(CHANNEL, acceptAllMessageFilter());
									owner.traceMessage(
											"[Audit2Timed] A : abonné à " + CHANNEL + "\n");
								} catch (Exception e) {
									owner.traceMessage(
											"[Audit2Timed] A : échec subscribe : " + e + "\n");
								}
							}),
					new TestStep(
							CLOCK_URI,
							...
							owner -> {
								try {
									...
									((PluginClient) owner)
											.subscribe(CHANNEL, acceptAllMessageFilter());
									owner.traceMessage(
											"[Audit2Timed] B : abonné à " + CHANNEL + "\n");
								} catch (Exception e) {
									owner.traceMessage(
											"[Audit2Timed] B : échec subscribe : " + e + "\n");
								}
							}),
					new TestStep(
							CLOCK_URI,
							...
							owner -> {
								try {
									...
									owner.traceMessage(
											"[Audit2Timed] B : Future getNextMessage() créé\n");
									Object msg = f.get(2, TimeUnit.SECONDS);
									owner.traceMessage(
											"[Audit2Timed] B : Future terminé => " + msg + "\n");
								} catch (Exception e) {
									owner.traceMessage(
											"[Audit2Timed] B : Future en échec/timeout : "
													+ e
													+ "\n");
								}
							}),
					new TestStep(
							...
							owner -> {
								try {
									...
									owner.traceMessage(
											"[Audit2Timed] A : publish envoyé (asynchrone)\n");
								} catch (Exception e) {
									owner.traceMessage(
											"[Audit2Timed] A : échec publish : " + e + "\n");
								}
							}),
					new TestStep(
							CLOCK_URI,
							...
							owner -> {
								try {
									...
									owner.traceMessage(
											"[Audit2Timed] B : publish envoyé (asynchrone)\n");
								} catch (Exception e) {
									owner.traceMessage(
											"[Audit2Timed] B : échec publish : " + e + "\n");
								}
							}),
					new TestStep(
							CLOCK_URI,
							...
							owner -> {
								try {
									System.out.println(
											"[Audit2Timed][STEP@"
													+ tTimedWait
													+ "] A : waitForNextMessage("
													+ CHANNEL
													+ ", 2s)");
									PluginClient c = (PluginClient) owner;
									Object msg =
											c.subscriptionPlugin.waitForNextMessage(
													CHANNEL, Duration.ofSeconds(2));
									owner.traceMessage(
											"[Audit2Timed] A : waitForNextMessage => "
													+ msg
													+ "\n");
								} catch (Exception e) {
									owner.traceMessage(
											"[Audit2Timed] A : échec timed wait : " + e + "\n");
								}
							})
				});
	}
	...
	public void deploy() throws Exception {
		...
		System.out.println(
				"[Audit2Timed] Déploiement du CVM : création du broker + clients + horloge"
					+ " accélérée + runner...\n");

		AbstractComponent.createComponent(
				Broker.class.getCanonicalName(), new Object[] {BROKER_URI, 4, 1, 3, 2, 5, 2, 4, 8});
