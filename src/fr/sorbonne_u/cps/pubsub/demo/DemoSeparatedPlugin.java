...
/**
 * Mid-semester timed integration scenario — tests the pub/sub SYSTEM only, independently of the
 * weather application.
 *
 * <p>Structure follows the BCM4Java rules: - Components are generic pub/sub clients (not
 * WindTurbine/WeatherStation) - Registration happens in each component's execute(), NOT in scenario
 * steps - Scenario steps only contain domain actions (subscribe, publish, createChannel) - Each
 * component receives the scenario via its constructor - executeTestScenario is called inside
 * execute()
 *
...
public class DemoSeparatedPlugin extends AbstractCVM {
	 ...
	 */
	public DemoSeparatedPlugin() throws Exception {
		super();
	}
	...
	/**
	 * Builds the test scenario. Registration is done in each component's execute() before
	 * executeTestScenario is called, so by the time the first scenario step fires, all components
				...
				new TestStepI[] {
							...
							owner -> {
								try {
									((PrivilegedClient) owner)
											.modifyServiceClass(RegistrationClass.STANDARD);
									((PrivilegedClient) owner)
					...
					new TestStep(
							...
							owner -> {
								try {
									((PrivilegedClient) owner)
											.modifyServiceClass(RegistrationClass.PREMIUM);
									((PrivilegedClient) owner)
											.traceMessage("upgraded to PREMIUM\n");
								} catch (Exception e) {
									((PrivilegedClient) owner)
											.logMessage("upgrade PREMIUM failed: " + e + "\n");
								}
							}),
								...
								try {
									String regex =
											"^("
													...
													+ "-reg)$";
									((PrivilegedClient) owner)
											.createChannel(CH_ALERTS_STD_PRIV, regex);
									((PrivilegedClient) owner)
											.traceMessage("created " + CH_ALERTS_STD_PRIV + "\n");
								} catch (Exception e) {
									((PrivilegedClient) owner)
					...
					new TestStep(
							...
							owner -> {
								try {
									((PrivilegedClient) owner)
											.createChannel("extra-should-fail", ".*");
									((PrivilegedClient) owner)
											.logMessage("ERROR: quota not enforced!\n");
								} catch (ChannelQuotaExceededException expected) {
									assert true : "quota exceeded as expected";
									((PrivilegedClient) owner)
											.traceMessage("quota exceeded as expected ✓\n");
								} catch (Exception e) {
									((PrivilegedClient) owner)
											.logMessage("unexpected exception: " + e + "\n");
								}
							}),
					new TestStep(
							TEST_CLOCK_URI,
								...
								try {
									String regex =
											"^("
													...
													+ "-reg)$";
									((PrivilegedClient) owner)
											.createChannel(CH_WIND_PREM_PRIV, regex);
									((PrivilegedClient) owner)
											.traceMessage("created " + CH_WIND_PREM_PRIV + "\n");
								} catch (Exception e) {
									((PrivilegedClient) owner)
											.logMessage(
													"createChannel premium failed: " + e + "\n");
								}
							}),
							...
							owner -> {
								try {
									...
									((SubscriberClient) owner)
											.traceMessage("subscribed to wind-free ✓\n");
								} catch (Exception e) {
									((SubscriberClient) owner)
											.logMessage("subscribe wind failed: " + e + "\n");
								}
							}),
									...
									MessageFilterI f =
											new MessageFilter(
													new PropertyFilterI[] {
														new PropertyFilter(
																"type",
																new EqualsValueFilter("alert")),
														new PropertyFilter(
																"level",
																ComparableValueFilter
																		.greaterOrEqual("ORANGE")),
														new PropertyFilter(
																"alertType",
																new MultiValuesFilter(
																		"STORM", "FLOOD"))
													},
													new MessageFilterI.PropertiesFilterI[0],
													new AfterOrAtTimeFilter(
															Instant.parse(START_INSTANT_STR)));
									((SubscriberClient) owner).subscribe(CH_ALERTS_STD_PRIV, f);
									((SubscriberClient) owner)
											.traceMessage("subscribed to alerts-priv ✓\n");
								} catch (Exception e) {
									((SubscriberClient) owner)
											.logMessage("subscribe alerts failed: " + e + "\n");
								}
							}),
					...
					new TestStep(
							...
							owner -> {
								try {
									...
									((SubscriberClient) owner)
											.logMessage(
													"ERROR: intruder should have been rejected!\n");
									...
									if (cause instanceof UnauthorisedClientException) {
										...
										((SubscriberClient) owner)
												.traceMessage("intruder rejected as expected ✓\n");
									} else {
										((SubscriberClient) owner)
												.logMessage("unexpected: " + e + "\n");
									}
								} catch (Exception e) {
									((SubscriberClient) owner)
											.logMessage("unexpected: " + e + "\n");
					...
					new TestStep(
							...
							owner -> {
								try {
									...
									((PublisherClient) owner)
											.traceMessage("published wind-near ✓\n");
								} catch (Exception e) {
									((PublisherClient) owner)
											.logMessage("publish wind-near failed: " + e + "\n");
								}
							}),
					...
					new TestStep(
							...
							owner -> {
								try {
									...
									((PublisherClient) owner)
											.traceMessage(
													"published wind-far (should be filtered by"
														+ " distance)\n");
								} catch (Exception e) {
									((PublisherClient) owner)
											.logMessage("publish wind-far failed: " + e + "\n");
								}
							}),
					...
					new TestStep(
							...
							owner -> {
								try {
									...
									((PublisherClient) owner)
											.traceMessage(
													"published bad-type (should be filtered)\n");
								} catch (Exception e) {
									((PublisherClient) owner)
											.logMessage("publish bad-type failed: " + e + "\n");
								}
							}),
					...
					new TestStep(
							...
							owner -> {
								try {
									...
									((PrivilegedClient) owner)
											.traceMessage("published ORANGE alert ✓\n");
								} catch (Exception e) {
									((PrivilegedClient) owner)
											.logMessage("publish orange failed: " + e + "\n");
								}
							}),
					...
					new TestStep(
							...
							owner -> {
								try {
									...
									((PrivilegedClient) owner)
											.traceMessage(
													"published GREEN alert (should be filtered)\n");
								} catch (Exception e) {
									((PrivilegedClient) owner)
											.logMessage("publish green failed: " + e + "\n");
								}
							}),
					...
					new TestStep(
							...
							owner -> {
								try {
									...
									((PrivilegedClient) owner)
											.traceMessage("published premium wind ✓\n");
								} catch (Exception e) {
									((PrivilegedClient) owner)
											.logMessage("publish premium failed: " + e + "\n");
								}
							}),
				});
	}
	...
	/**
	 * Filter that accepts a value if it belongs to a predefined set of strings. Extracted as a
	 * proper named class (not anonymous) for clarity.
	 */
	public static final class MultiValuesFilter implements MessageFilterI.ValueFilterI {
		 ...
		 * @return {@code true} ssi {@code value.toString()} appartient à l'ensemble fourni au
		 *     constructeur.
		 */
		@Override
