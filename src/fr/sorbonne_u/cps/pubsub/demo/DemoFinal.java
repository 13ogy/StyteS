...
/**
 * Test d'intégration distribué final — 4 JVMs, 4 brokers, gossip protocol fully exercised.
 *
 ...
 * </pre>
 *
 * <p><strong>Gossip messages exercised</strong>
 *
 * <ul>
 *   <li>{@code RegisterGossipMessage} — all 8 clients cross-JVM
 *   <li>{@code ModifyServiceClassGossipMessage} — C3/C5 upgrade
 *   <li>{@code CreateChannelGossipMessage} — privileged channels
 *   <li>{@code PublishGossipMessage} — cross-JVM delivery
 *   <li>{@code ModifyAuthorisedUsersGossipMessage} — C5 widens access
 *   <li>{@code DestroyChannelGossipMessage} — C3 destroys its channel
 *   <li>{@code UnregisterGossipMessage} — C6 unregisters
 * </ul>
 *
 * <p><strong>Launch sequence (5 terminals)</strong>
 *
 * <pre>
 * # 1. Global registry
...
public class DemoFinal extends AbstractDistributedCVM {
	 ...
	 */
	public DemoFinal(String[] args) throws Exception {
		super(args);
	}
	 ...
	 *
	 * <p>All registrations happen in each component's {@code execute()} method before {@code
	 * executeTestScenario} is called. By the time the first step fires, every client is already
	 * registered at its local broker, and the initial {@code RegisterGossipMessage}s have had time
	 * to propagate.
	 *
	 * <p><strong>Timeline (ACCELERATION_FACTOR = 1.0)</strong>
	 *
	 * <pre>
	...
	public static TestScenario buildScenario() throws Exception {
		...
		String stdRegex = "^(" + C1_SUB + "-reg|" + C4_SUB + "-reg|" + C7_SUB + "-reg)$";
		...
		String premRegex2 = "^(" + C1_SUB + "-reg|" + C4_SUB + "-reg|" + C7_SUB + "-reg)$";
				...
				new TestStepI[] {
					...
					new TestStep(
							...
							owner -> {
								try {
									...
									owner.traceMessage(
											"[C3] created '" + CH_STD + "' (gossip→all brokers)\n");
								} catch (Exception e) {
							...
							owner -> {
								try {
									((SubscriberClient) owner)
											.subscribe(CH_STD, MessageFilter.acceptAll());
									...
									if (cause instanceof UnauthorisedClientException) {
										owner.traceMessage(
												"[C8] correctly rejected from '"
														+ CH_STD
														+ "' ✓\n");
									} else {
										owner.traceMessage(
												"[C8] unexpected exception: " + e + "\n");
									}
								} catch (Exception e) {
					...
					new TestStep(
							...
							owner -> {
								try {
									((PrivilegedClient) owner)
											.modifyAuthorisedUsers(CH_PREM, premRegex2);
									owner.traceMessage(
											"[C5] widened '"
													+ CH_PREM
													+ "' authorisation (gossip"
													+ " ModifyAuthorisedUsers)\n");
								} catch (Exception e) {
									owner.traceMessage(
											"[C5] modifyAuthorisedUsers failed: " + e + "\n");
								}
							}),
					...
					new TestStep(
							...
							owner -> {
								try {
									((SubscriberClient) owner)
											.subscribe(CH_PREM, MessageFilter.acceptAll());
									owner.traceMessage(
											"[C4] subscribed to '"
													+ CH_PREM
							...
							owner -> {
								try {
									...
									if (cause instanceof UnknownChannelException) {
										owner.traceMessage(
												"[C3] correctly rejected from publishing on"
													+ " destroyed channel✓\n");
									} else {
										owner.traceMessage(
												"[C3] unexpected exception: " + e + "\n");
									}
								} catch (Exception e) {
					...
					new TestStep(
							...
							owner -> {
								try {
									...
									owner.traceMessage(
											"[C6] unregistered (gossip Unregister→all)\n");
								} catch (Exception e) {
	...
	public void instantiateAndPublish() throws Exception {
		...
		if (thisJVMURI.equals(JVM1)) {
			...
			long unixStart =
					TimeUnit.MILLISECONDS.toNanos(System.currentTimeMillis() + DELAY_TO_START_MS);
			AbstractComponent.createComponent(
					ClocksServer.class.getCanonicalName(),
					new Object[] {
						CLOCK_URI, unixStart, Instant.parse(START_INSTANT_STR), ACCELERATION_FACTOR
					});
	...
	private void createBroker(String uri, List<String> gossipNeighbors) throws Exception {
		AbstractComponent.createComponent(
	...
	private void createSubscriber(
			String uri, String brokerURI, TestScenario ts, RegistrationClass rc) throws Exception {
		AbstractComponent.createComponent(
				SubscriberClient.class.getCanonicalName(), new Object[] {uri, brokerURI, ts, rc});
	}

	private void createPublisher(
			String uri, String brokerURI, TestScenario ts, RegistrationClass rc) throws Exception {
		AbstractComponent.createComponent(
				PublisherClient.class.getCanonicalName(), new Object[] {uri, brokerURI, ts, rc});
	}

	private void createPrivileged(
			String uri, String brokerURI, TestScenario ts, RegistrationClass rc) throws Exception {
		AbstractComponent.createComponent(
				PrivilegedClient.class.getCanonicalName(), new Object[] {uri, brokerURI, ts, rc});
	}
