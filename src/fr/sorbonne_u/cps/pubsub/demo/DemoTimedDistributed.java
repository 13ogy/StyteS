...
/**
 * Distributed CVM demonstrating the gossip protocol between two brokers across two separate JVMs.
 *
 * <p><strong>Description</strong>
 *
 * <p>This deployment validates the gossip protocol (CDC §3.7) by running two broker components on
 * separate JVMs, each hosting one client component:
 *
 * <ul>
 *   <li><strong>JVM {@value #BROKER1_JVM_URI}</strong> — Broker B1 + {@link SubscriberClient} C1
 *       (FREE class) + {@link ClocksServer}
 *   <li><strong>JVM {@value #BROKER2_JVM_URI}</strong> — Broker B2 + {@link PrivilegedClient} C2
 *       (starts FREE, upgrades to STANDARD)
 * </ul>
 *
 * <p><strong>Test scenario</strong>
 *
 * <ol>
 *   <li>C1 and C2 register at their respective local brokers; registration is propagated via {@code
 *       RegisterGossipMessage}.
 *   <li>C2 upgrades to STANDARD via {@code modifyServiceClass}.
 *   <li>C2 creates a privileged channel {@value #CHANNEL} restricted to C1; creation is propagated
 *       via {@code CreateChannelGossipMessage} so B1 replicates the channel locally.
 *   <li>C1 subscribes to {@value #CHANNEL} on B1.
 *   <li>C2 publishes a message on {@value #CHANNEL} via B2; B2 validates and propagates a {@code
 *       PublishGossipMessage} to B1, which delivers the message to C1.
 * </ol>
 *
...
public class DemoTimedDistributed extends AbstractDistributedCVM {
	 ...
	 * </pre>
	 *
	 * @param args command-line arguments: {@code args[0]} is the URI of this JVM in the assembly;
	 *     {@code args[1]} is the path to the XML configuration file.
	...
	@Override
	/**
	 * Hook d'initialisation BCM appelé avant {@link #instantiateAndPublish()} : point d'entrée
	 * standard pour activer les modes de debug.
	 *
	 ...
	 *
	 * <p>Steps (all instants relative to {@value #START_INSTANT_STR}, real-time with {@code
	 * ACCELERATION_FACTOR = 1.0}):
	 *
	 * <ol>
	 *   <li><strong>t+5 s</strong> — C2 upgrades to STANDARD.
	 *   <li><strong>t+15 s</strong> — C2 creates privileged channel {@value #CHANNEL}; gossip
	 *       propagates to B1.
	 *   <li><strong>t+25 s</strong> — C1 subscribes to {@value #CHANNEL} on B1 (channel is now
	 *       known locally via gossip).
	 *   <li><strong>t+35 s</strong> — C2 publishes a message; B2 sends a {@code
	 *       PublishGossipMessage} to B1 which delivers it to C1.
	 * </ol>
	 *
	...
	public static TestScenario testScenario() throws Exception {
		...
		Instant publishC2Instant =
				startInstant.plusSeconds(35); // après gossip RegisterGossip propagé
		...
		return new TestScenario(
				...
				new TestStepI[] {
					...
					new TestStep(
							...
							owner -> {
								try {
									...
									((PrivilegedClient) owner)
											.traceMessage("upgraded to STANDARD\n");
								} catch (Exception e) {
									((PrivilegedClient) owner)
											.logMessage("upgrade STANDARD failed: " + e + "\n");
								}
							}),
					...
					new TestStep(
							CLOCK_URI,
							CLIENT2_URI,
							createChannelC2Instaant,
							owner -> {
								try {
									String regex = "^(" + CLIENT1_URI + "-reg)$";
									((PrivilegedClient) owner).createChannel(CHANNEL, regex);
									((PrivilegedClient) owner)
											.traceMessage("created " + CHANNEL + "\n");
								} catch (Exception e) {
									((PrivilegedClient) owner)
											.logMessage("createChannel failed: " + e + "\n");
								}
							}),
					...
					new TestStep(
							...
							owner -> {
								try {
									...
									System.out.println(
											"[Test gossip] C2 published — returned immediately");
									((PrivilegedClient) owner)
											.traceMessage(
													"[Test gossip] C2: published message "
															+ m.getPayload()
															+ "\n");
								} catch (Exception e) {
									owner.traceMessage(
											"[Test gossip] C2: publish failed: " + e + "\n");
								}
							}),
				});
	}
	 ...
	 *
	 * <p>On {@value #BROKER1_JVM_URI}: creates Broker B1 (gossip neighbour = B2), SubscriberClient
	 * C1, and the shared ClocksServer.<br>
	 * On {@value #BROKER2_JVM_URI}: creates Broker B2 (gossip neighbour = B1) and PrivilegedClient
	 * C2.
	 *
	...
	public void instantiateAndPublish() throws Exception {
		...
		if (thisJVMURI.equals(BROKER1_JVM_URI)) {
			...
			AbstractComponent.createComponent(
					Broker.class.getCanonicalName(),
					new Object[] {
						BROKER2_URI, 2, 1, 3, 2, 5, 2, 4, 8, broker2Neighbors // URI gossip de B1
					});
	...
	@Override
	/**
	 * Établit les connexions inter-JVM gossip et clients ↔ brokers une fois tous les composants
	 * publiés.
	 *
	 ...
	 * Entry point.
	 *
	 * <p>Starts the standard BCM4Java life cycle with a total budget of 120 seconds ({@value
	 * #DELAY_TO_START_MS} ms startup delay + 100 s scenario + safety margin).
	 *
