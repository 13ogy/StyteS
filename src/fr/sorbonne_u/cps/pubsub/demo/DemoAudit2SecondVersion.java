 ...
 *
 * <p>Démontre : - Appels asynchrones pour la publication (pipeline broker) - Parallélisme via trois
 * pools de threads distincts : 5 clients publient au même instant → 5 messages traversent le
 * pipeline simultanément → esReceptionIndex reçoit 5 tâches en parallèle → esPropagationIndex
 * propage 5 messages vers 5 abonnés en parallèle → esDeliveryIndex livre 25 combinaisons (5 msgs ×
 * 5 abonnés) en parallèle - Concurrence maîtrisée dans le broker (sections critiques) - Modes de
 * réception avancés : getNextMessage (Future) et waitForNextMessage(Duration)
 *
				...
				new TestStepI[] {
					...
					new TestStep(
							...
							owner -> {
								try {
									System.out.println(
											"[Audit2] B calling getNextMessage() — non-blocking");
									owner.traceMessage("[Audit2] B: getNextMessage() called\n");
									Future<MessageI> f =
											((FullClient) owner).getNextMessage(CHANNEL);
									...
									owner.traceMessage(
											"[Audit2] B: Future failed/timeout: " + e + "\n");
								}
							}),
					...
					new TestStep(
							...
							owner -> {
								try {
									...
									System.out.println(
											"[Audit2] A published async — returned immediately");
									owner.traceMessage(
											"[Audit2] A: publish submitted asynchronously\n");
								} catch (Exception e) {
					...
					new TestStep(
							...
							owner -> {
								try {
									...
									((FullClient) owner).publish(CHANNEL, m);
									System.out.println(
											"[Audit2] B published async — returned immediately");
									owner.traceMessage(
											"[Audit2] B: publish submitted asynchronously\n");
								} catch (Exception e) {
					...
					new TestStep(
							...
							owner -> {
								try {
									...
									((FullClient) owner).publish(CHANNEL, m);
									System.out.println(
											"[Audit2] C published async — returned immediately");
									owner.traceMessage(
											"[Audit2] C: publish submitted asynchronously\n");
								} catch (Exception e) {
					...
					new TestStep(
							...
							owner -> {
								try {
									...
									((FullClient) owner).publish(CHANNEL, m);
									System.out.println(
											"[Audit2] D published async — returned immediately");
									owner.traceMessage(
											"[Audit2] D: publish submitted asynchronously\n");
								} catch (Exception e) {
					...
					new TestStep(
							...
							owner -> {
								try {
									...
									((FullClient) owner).publish(CHANNEL, m);
									System.out.println(
											"[Audit2] E published async — returned immediately");
									owner.traceMessage(
											"[Audit2] E: publish submitted asynchronously\n");
								} catch (Exception e) {
					...
					new TestStep(
							...
							owner -> {
								try {
									System.out.println(
											"[Audit2] A calling waitForNextMessage(10s)");
									MessageI msg =
											((FullClient) owner)
													.waitForNextMessage(
															CHANNEL, Duration.ofSeconds(10));
									if (msg != null) {
										System.out.println(
												"[Audit2] A received: " + msg.getPayload());
										owner.traceMessage(
												"[Audit2] A: waitForNextMessage => "
														+ msg.getPayload()
														+ "\n");
									} else {
										System.out.println("[Audit2] A: timeout");
										owner.traceMessage(
												"[Audit2] A: waitForNextMessage => timeout\n");
									}
								} catch (Exception e) {
									owner.traceMessage(
											"[Audit2] A: waitForNextMessage failed: " + e + "\n");
								}
							}),
				});
	}
