 ...
 *
 * <p>Ce plugin <em>possède son propre port outbound {@code PublishingCI}</em> . Le design précédent
 * centralisait tous les ports outbound dans le {@link ClientRegistrationPlugin}, ce qui forçait un
 * composant qui ne souscrit qu'à un seul rôle (par ex. un publieur pur) à instancier malgré tout
 * les ports privilégiés et de souscription qu'il n'utilisera jamais. Conformément aux exemples
 * {@code SimpleRingOverlayNetwork-CM9} (un greffon = un port) et {@code ProducerConsumer-13032026}
 * (greffons producteur/consommateur disjoints), chaque greffon possède désormais le port qui sert
 * son rôle.
 *
 * <p>L'identité du client transmise au broker reste l'URI du port {@code ReceivingCI} obtenue via
 * {@link ClientRegistrationPlugin#getReceptionPortURI()} : ce greffon ne possède pas de port
 * inbound mais a besoin du {@link ClientRegistrationPlugin} pour récupérer cette identité ainsi que
 * le port outbound {@code RegistrationCI} utilisé par {@link #channelExist(String)} et {@link
 * #channelAuthorised(String)}.
 *
 * <p>Cycle de vie BCM :
 *
 * <ul>
 *   <li>{@link #installOn} — déclare {@code PublishingCI} comme requise (idempotent) ;
 *   <li>{@link #initialise} — publie le port et le connecte au port inbound {@code PublishingCI} du
 *       broker, dont l'URI est dérivée déterministement via {@link
 *       Broker#publishingPortURIFor(String)} ;
 ...
 * </ul>
 *
 * <p>L'URI du plugin suit la convention {@code <reflectionURI>-publication-plugin}.
 *
...
public class ClientPublicationPlugin extends AbstractPlugin implements ClientPublicationI {
	...
	/**
	 * Plugin de registration : fournit l'URI {@code ReceivingCI} (identité du client) et le port
	 * outbound {@code RegistrationCI} pour {@code channelExist} / {@code channelAuthorised}.
	 */
	protected final ClientRegistrationPlugin registrationPlugin;

	/**
	 * URI de réflexion du broker, utilisée pour dériver l'URI du port inbound {@code PublishingCI}
	 * via {@link Broker#publishingPortURIFor(String)}.
	 */
	...
	/**
	 * {@code true} ssi {@link #installOn} a effectivement déclaré {@code PublishingCI} sur le
	 * composant ; sinon, l'interface était déjà déclarée (par exemple via une annotation
	 * {@code @RequiredInterfaces} sur le composant) et le greffon ne doit pas la retirer dans
	 ...
	 *     et non vide.
	 */
	public ClientPublicationPlugin(
	...
	/**
	 * Publie le port outbound {@code PublishingCI} et le connecte au port inbound correspondant du
	 * broker (URI dérivée via {@link Broker#publishingPortURIFor(String)}).
	 *
	 * <p>Le greffon se connecte avant tout appel à {@link ClientRegistrationPlugin#register} : la
	 * connexion ne donne <em>aucune</em> permission ; les permissions sont vérifiées par le broker
	 * à chaque appel via {@code requireRegistered(rip)} et, si besoin, {@code
	 * requireServiceClass(rip, …)}.
	 *
	 * @throws Exception si la publication ou la connexion d'un port échoue, ou si l'URI du broker
	 *     n'a pas été fournie au constructeur.
	 */
	@Override
	 ...
	 * retire {@code PublishingCI} si elle a été déclarée par {@link #installOn}.
	 *
	 ...
	 * {@inheritDoc}
	 *
	 * <p>Délègue au port {@code RegistrationCI} du greffon de registration (le canal {@code
	 * channelExist} fait partie de {@code RegistrationCI} dans ce projet, voir l'interface frozen).
	 *
	 ...
	 *
	 * @param channel nom du canal interrogé.
	 * @return {@code true} ssi ce client est autorisé à publier sur {@code channel}.
	...
	/**
	 * Publie {@code message} sur {@code channel} via le port outbound {@code PublishingCI} possédé
	 * par ce greffon.
	 *
	...
	/**
	 * Soumet la publication via {@code runTask} sur l'un des executors du composant propriétaire
	 * pour ne pas bloquer le thread appelant.
	 *
	 * @param channel canal de publication.
	 * @param message message à publier.
	 */
