...
/**
 * Composant "Station météo" (CDC §3.4) modélisé comme un client pub/sub <strong>publieur</strong>
 * spécialisé.
 *
 * <p><strong>Choix de conception</strong> : <em>WeatherStation</em> hérite de {@link
 * PublisherClient}, qui fournit déjà les greffons {@link
 * fr.sorbonne_u.cps.pubsub.plugins.ClientRegistrationPlugin} + {@link
 * fr.sorbonne_u.cps.pubsub.plugins.ClientPublicationPlugin} ainsi que tout le câblage de cycle de
 * vie. Le composant n'embarque que sa logique métier (fabrication des messages vent +
 * identifiants/position de la station). Cela évite la duplication du wiring BCM4Java et garantit
 * que tous les publieurs FREE partagent le même contrat.
 *
...
public class WeatherStation extends PublisherClient {
	 ...
	 */
	protected WeatherStation(
			String uri, String stationId, PositionI position, String brokerReflectionURI)
			throws Exception {
	...
	/**
	 * Publie une observation de vent sur le canal indiqué. Le message est construit via {@link
	 * WindMessageFactory#build(String, WindDataI)} (CDC §3.5 — convention de propriétés des
	 * messages vent).
	 *
	 * @param windChannel canal de publication (typiquement {@link
	 *     MeteoProperties#DEFAULT_WIND_CHANNEL}).
	...
	public void publishWind(String windChannel, WindDataI wind) throws Exception {
		...
		String out =
				"WeatherStation[" + stationId + "] publish wind " + wind + " on " + windChannel;
