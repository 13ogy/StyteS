...
/**
 * Composant "Bureau météo" (CDC §3.4) modélisé comme un client pub/sub <strong>privilégié</strong>
 * spécialisé.
 *
 * <p><strong>Choix de conception</strong> : <em>WeatherOffice</em> hérite de {@link
 * PrivilegedClient}, qui fournit {@link fr.sorbonne_u.cps.pubsub.plugins.ClientRegistrationPlugin}
 * + {@link fr.sorbonne_u.cps.pubsub.plugins.ClientPrivilegedPlugin} (canal privilégié = inclut
 * publication, cf. CDC §3.6). Le composant n'apporte que la fabrication des messages d'alerte et la
 * promotion automatique en classe {@code STANDARD} (le bureau a besoin des canaux privés pour
...
public class WeatherOffice extends PrivilegedClient {
	...
	protected WeatherOffice(
			String uri, TestScenario testScenario, String officeId, String brokerReflectionURI)
			throws Exception {
	 ...
	 */
	protected WeatherOffice(String uri, String officeId, String brokerReflectionURI)
			throws Exception {
	...
	/**
	 * Le parent {@link PrivilegedClient#execute()} effectue l'enregistrement FREE puis exécute le
	 * scénario éventuel ; on enchaîne ici la promotion vers la classe {@code STANDARD} pour activer
	 * les fonctionnalités privilégiées attendues du bureau (création de canaux privés et
	...
	/**
	 * Publie une alerte météo sur le canal indiqué. Le message est construit via {@link
	 * MeteoAlertMessageFactory#build(String, MeteoAlertI)} (CDC §3.5 — convention de propriétés des
	 * messages d'alerte).
	 *
	 * @param alertChannel canal de publication (typiquement {@link
	 *     MeteoProperties#DEFAULT_ALERT_CHANNEL} ou un canal privilégié appartenant au bureau).
	...
	public void publishAlert(String alertChannel, MeteoAlertI alert) throws Exception {
		...
		String out =
				"WeatherOffice[" + officeId + "] publish alert " + alert + " on " + alertChannel;
