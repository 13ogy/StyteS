...
{
	private MeteoFilters() { /* factory only */ }

	/**
	 * Filter accepting wind messages whose {@link MeteoProperties#PAYLOAD} is
	 * within {@code maxDistance} of {@code reference}.
	 */
	public static MessageFilterI windWithinDistance(PositionI reference, double maxDistance)
	{
		return new MessageFilter(
			new MessageFilterI.PropertyFilterI[] {
				new PropertyFilter(MeteoProperties.TYPE, new EqualsValueFilter(MeteoProperties.TYPE_WIND)),
				new PropertyFilter(MeteoProperties.PAYLOAD, new DistanceWindFilter(reference, maxDistance))
			},
	...
	public static MessageFilterI anyAlert()
	{
		return new MessageFilter(
			new MessageFilterI.PropertyFilterI[] {
				new PropertyFilter(MeteoProperties.TYPE, new EqualsValueFilter(MeteoProperties.TYPE_ALERT))
			},
			new MessageFilterI.PropertiesFilterI[0],
			new AcceptAllTimeFilter());
	}
	...
	/**
	 * Filter accepting alerts whose {@link MeteoProperties#LEVEL} is at least
	 * the provided minimum (string-comparison on the level name; works because
	 * {@link MeteoAlertI.Level} names are alphabetically meaningful for the
	 * existing demo, but callers should prefer {@link #atLeastAlertLevel} when
	 * comparing on the enum directly).
	 */
	public static MessageFilterI alertWithLevelAtLeastByName(String minimumLevelName)
	{
		return new MessageFilter(
			new MessageFilterI.PropertyFilterI[] {
				new PropertyFilter(MeteoProperties.TYPE, new EqualsValueFilter(MeteoProperties.TYPE_ALERT)),
				new PropertyFilter(MeteoProperties.LEVEL,
