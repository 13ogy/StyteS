...
/**
 * Composant "Éolienne" (CDC §3.4) modélisé comme un client pub/sub <strong>souscripteur</strong>
 * spécialisé.
 *
 * <p><strong>Choix de conception</strong> : <em>WindTurbine</em> hérite de {@link
 * SubscriberClient}, le composant client générique qui possède déjà les greffons {@link
 * fr.sorbonne_u.cps.pubsub.plugins.ClientRegistrationPlugin} et {@link
 * fr.sorbonne_u.cps.pubsub.plugins.ClientSubscriptionPlugin}. On évite ainsi de redupliquer le
 * câblage des plugins (installation, connexion, déconnexion, propre {@code start}/{@code shutdown})
 * déjà présent dans la classe parente. Le composant ne contient plus que la logique métier
 * (filtrage géographique, dispatch wind/alert, mode sécurité). Cette factorisation s'inscrit dans
 * le principe DRY.
 *
 * <p>Le composant reçoit des observations de vent ({@link WindDataI}) et des alertes ({@link
 * MeteoAlertI}). Les observations de vent sont filtrées par distance côté courtier via {@link
 * MeteoFilters#windWithinDistance(PositionI, double)}, ce qui <em>déporte</em> le filtrage
 * géographique vers le système pub/sub plutôt que de transporter inutilement les messages éloignés.
 *
 * <p>Sur réception d'une alerte concernant la région de l'éolienne, le composant bascule en mode
 * "safety" lorsque le niveau atteint le seuil configuré, et revient à la normale sur niveau {@link
...
public class WindTurbine extends SubscriberClient {
	...
	/**
	 * Constructeur principal. La signature respecte la convention {@code (uri, scenario?,
	 * turbineId, position, maxDistance, recentWindowMillis, threshold, brokerReflectionURI)}
	 * utilisée par les démos centralisées.
	 *
	...
	/**
	 * Surcharge sans scénario, pour les déploiements répartis (cf. {@link
	 * fr.sorbonne_u.cps.pubsub.demo.DemoTimedDistributed}).
	 *
	...
	/**
	 * Enregistre le composant (via le parent) puis, en l'absence de scénario, souscrit aux canaux
	 * par défaut afin de rester utilisable dans les déploiements simples (sans {@code
	 * TestScenario}).
	 *
	 * @throws Exception si l'enregistrement, l'horloge ou la souscription échoue.
	 */
	@Override
	public void execute() throws Exception {
		super.execute();
		if (this.autoSubscribeDefaults) {
			subscribeToWindAndAlerts(
					MeteoProperties.DEFAULT_WIND_CHANNEL, MeteoProperties.DEFAULT_ALERT_CHANNEL);
		}
	}
	...
	/**
	 * Souscrit aux observations de vent (filtrées par distance) et à toutes les alertes météo, en
	 * réutilisant la méthode {@code subscribe} héritée du parent (qui passe par le greffon de
	 * souscription).
	 *
	 * @param windChannel canal des observations de vent.
	 * @param alertChannel canal des alertes.
	 * @throws Exception si l'enregistrement des souscriptions auprès du courtier échoue.
	 */
	public void subscribeToWindAndAlerts(String windChannel, String alertChannel) throws Exception {
		MessageFilterI windFilter =
				MeteoFilters.windWithinDistance(this.position, this.maxDistance);
	...
	/**
	 * Hook de livraison appelé par le greffon de souscription pour chaque message reçu sur l'un des
	 * canaux souscrits. <em>Override</em> du comportement neutre du parent : le dispatch se fait
	 * par <strong>nom de canal</strong> (et non par {@code instanceof} sur le payload), ce qui
	 * découple la dispatch du type concret de message.
	 *
	...
	public void onReceive(String channel, MessageI message) {
		try {
			if (channel.equals(this.windChannel)) {
				onWind((WindDataI) message.getPayload(), message.getTimeStamp().toEpochMilli());
			...
			this.logMessage(
					"WindTurbine[" + turbineId + "] receive failed: " + e.getMessage() + "\n");
		}
	}
	...
	private void onAlert(MeteoAlertI alert) {
		...
		if (!concerned) {
			this.traceMessage(
					"WindTurbine[" + turbineId + "] ignore alert (not concerned): " + alert + "\n");
			return;
		}

		MeteoAlertI.LevelI rawLevel = alert.getLevel();
		MeteoAlertI.Level level =
				(rawLevel instanceof MeteoAlertI.Level) ? (MeteoAlertI.Level) rawLevel : null;
