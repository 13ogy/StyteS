...
/**
 * Concrete visitor that applies incoming gossip messages to a {@link Broker}'s local state.
 *
 * <p>Each {@code visit} method mirrors one operation that may have been performed on a remote
 * broker — registration, channel creation, publication, etc. — and replicates its effect locally,
 * ensuring consistency across the gossip network.
 *
 * <p>This class accesses the broker's internal state directly and must therefore reside in the same
 * package as {@link Broker}.
 *
...
public class BrokerGossipHandler implements GossipMessageVisitor {
	...
	public void visit(RegisterGossipMessage regMsg) {
		...
		try {
			broker.registeredClients.putIfAbsent(
					regMsg.getClientReceptionPortURI(), regMsg.getRegistrationClass());
			broker.createdPrivilegedChannelsCount.putIfAbsent(
					regMsg.getClientReceptionPortURI(), 0);
		} finally {
	...
	public void visit(PublishGossipMessage pubMsg) {
		...
		broker.runTask(
				...
				o -> {
					try {
						((Broker) o).propagationStage(pubMsg.getChannel(), pubMsg.getPubMessage());
					} catch (Exception e) {
	...
	public void visit(CreateChannelGossipMessage ccMsg) {
		broker.logMessage(
				"[Broker] gossip recv channel creation from " + ccMsg.getEmitterURI() + "\n");
		...
		try {
			...
			broker.registeredClients.putIfAbsent(
					ccMsg.getOwnerReceptionPortURI(), ccMsg.getOwnerClass());
	...
	public void visit(DestroyChannelGossipMessage dcMsg) {
		broker.logMessage(
				"[Broker] gossip recv destroy channel from " + dcMsg.getEmitterURI() + "\n");
	...
	public void visit(ModifyServiceClassGossipMessage mscMsg) {
		broker.logMessage(
				"[Broker] gossip recv modify service class from " + mscMsg.getEmitterURI() + "\n");

		broker.registrationLock.writeLock().lock();
		try {
			// Mettre à jour la classe de service localement
			broker.registeredClients.put(
					mscMsg.getClientReceptionPortURI(), mscMsg.getNewRegistrationClass());

			// Si downgrade vers FREE — supprimer le quota
			if (mscMsg.getNewRegistrationClass() == RegistrationCI.RegistrationClass.FREE) {
				broker.createdPrivilegedChannelsCount.remove(mscMsg.getClientReceptionPortURI());
			} else {
	...
	public void visit(ModifyAuthorisedUsersGossipMessage mauMsg) {
		broker.logMessage(
				"[Broker] gossip recv modify authorised users from "
						+ mauMsg.getEmitterURI()
						+ "\n");

		Pattern newPattern = Pattern.compile(mauMsg.getAuthorisedUsers());
		broker.channelsLock.writeLock().lock();
		try {
			Broker.PrivilegedChannelInfo info = broker.privilegedChannels.get(mauMsg.getChannel());
			if (info != null) {
	...
	public void visit(UnregisterGossipMessage unregMsg) {
		broker.logMessage(
				"[Broker] gossip recv unregister from " + unregMsg.getEmitterURI() + "\n");
