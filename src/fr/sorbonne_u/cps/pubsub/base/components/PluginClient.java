 ...
 *
 * <p>Ce composant compose explicitement quatre plugins (cf. paquetage {@code
 * fr.sorbonne_u.cps.pubsub.plugins}) plutôt que d'agréger manuellement les ports :
 *
 * <ul>
 *   <li>{@link ClientRegistrationPlugin} — porte le port inbound {@link ReceivingCI} et le port
 *       outbound {@link RegistrationCI} ; toutes les autres opérations passent par lui pour
 *       récupérer l'URI du port de réception (qui sert d'identité du client) ;
 *   <li>{@link ClientSubscriptionPlugin} — souscriptions, modification de filtre et API de
 *       réception avancée ;
 *   <li>{@link ClientPublicationPlugin} — port outbound {@link PublishingCI} ;
 *   <li>{@link ClientPrivilegedPlugin} — port outbound {@link PrivilegedClientCI} pour la gestion
 *       des canaux privilégiés.
 * </ul>
 *
 * <p>Pour un composant qui n'a besoin que d'un sous-ensemble de rôles, préférer {@link
 * SubscriberClient}, {@link PublisherClient}, {@link PrivilegedClient} ou {@link
 * AbstractPluginComponent} (composant abstrait spécialisable).
 *
...
public class PluginClient extends AbstractComponent {
	...
	/**
	 * @deprecated use {@link #PluginClient(String, int, int, String)} so that this client knows
	 *     which broker to connect to in a multi-broker environment.
	 */
	@Deprecated
	protected PluginClient(String reflectionInboundPortURI, int nbThreads, int nbSchedulableThreads)
			throws Exception {
		this(reflectionInboundPortURI, nbThreads, nbSchedulableThreads, null);
	}

	/**
	 * Preferred constructor: also takes the {@code brokerReflectionURI} so the registration plugin
	 * knows which broker to contact.
	 */
	...
	/**
	 * Demande au broker un changement de classe de service (upgrade ou downgrade vers FREE).
	 *
	 * @throws UnknownClientException si le client n'est pas enregistré.
	 * @throws AlreadyRegisteredException si la nouvelle classe est identique à la classe actuelle.
	 */
	...
	/**
	 * Désenregistre ce client (libère ses canaux privilégiés et ses souscriptions côté broker).
	 *
	 * @throws UnknownClientException si le client n'est pas enregistré.
	 */
	...
	/**
	 * Souscrit ce client au canal {@code channel} avec le filtre {@code filter} (qui peut être
	 * {@code null} pour « accepter tout »).
	 */
	 ...
	 * @param authorisedUsers regex appliquée aux URIs de port de réception des clients autorisés
	 *     (peut être vide pour « tout client enregistré »).
	 */
	...
	/**
	 * Détruit un canal privilégié possédé. Asynchrone : le retour ne garantit pas que la
	 * destruction soit visible immédiatement (cf. {@link #destroyChannelNow}).
	 */
	...
	/**
	 * Variante synchrone de {@link #destroyChannel} : la destruction est achevée au retour de la
	 * méthode.
	 */
	...
	/**
	 * Callback de réception câblé dans le {@link ClientSubscriptionPlugin}. Implémentation par
	 * défaut : trace + log. Les sous-classes peuvent remplacer ce comportement (file d'attente,
	 * filtrage applicatif, etc.).
	 */
