...
/**
 * Composant client de démonstration jouant le rôle de <strong>souscripteur pur</strong> :
 * enregistré au broker (par défaut FREE) puis souscrit à des canaux. Compose deux plugins :
 *
 * <ul>
 *   <li>{@link ClientRegistrationPlugin} — possède le port {@link
 *       fr.sorbonne_u.cps.pubsub.interfaces.ReceivingCI} entrant et le port {@link
 *       fr.sorbonne_u.cps.pubsub.interfaces.RegistrationCI} sortant ;
 *   <li>{@link ClientSubscriptionPlugin} — méthodes {@code subscribe} / {@code unsubscribe} ;
 *       reçoit les messages via le hook {@link #onReceive(String, MessageI)}.
 * </ul>
 *
 * <p>Les démos centralisées passent en plus un {@link TestScenario} ; en déploiement distribué (cf.
 * {@link fr.sorbonne_u.cps.pubsub.demo.DemoFinal}, {@link
 * fr.sorbonne_u.cps.pubsub.demo.DemoTimedDistributed}) le scénario peut être {@code null}.
 *
 * <p><strong>Convention constructeur</strong> : {@code (uri, brokerReflectionURI)} où {@code uri}
 * sert à la fois comme URI de port de réflexion BCM et comme identifiant participant dans les
 * {@link TestScenario}. Les surcharges 4-arg (avec scénario + classe initiale) sont utilisées par
 * les démos.
 *
...
public class SubscriberClient extends AbstractComponent {
	...
	/**
	 * Construit un souscripteur attaché à un broker, avec un scénario temporisé optionnel.
	 *
	...
	/**
	 * Constructeur sans scénario, pour les déploiements répartis (cf. {@link
	 ...
	 */
	protected SubscriberClient(
			String uri, String brokerReflectionURI, RegistrationCI.RegistrationClass rc)
			throws Exception {
		this(uri, brokerReflectionURI, null, rc);
	}

	/**
	 * Hook de réception (passé en {@code MessageDeliveryHandler} au {@link
	 * ClientSubscriptionPlugin}). Trace + log le message reçu.
	 *
	...
	/**
	 * Enregistre le client à la classe initiale, initialise l'horloge accélérée puis exécute le
	...
	/**
	 * Souscrit au canal donné avec le filtre fourni (action métier appelée depuis les étapes du
	 * scénario).
	 *
