...
/**
 * Composant client de démo (TEST-ONLY) : un {@link PluginClient} qui exécute les étapes du {@link
 * TestScenario} qui lui sont assignées via son URI de port de réflexion. Utilisé dans les démos
 * temporisées (audit 2, mid-sem complexe…) comme participant générique.
 *
 * <p>Particularité BCM4Java : {@code executeTestScenario(...)} ne planifie que les étapes du
 * composant qui l'appelle ; chaque participant doit donc appeler {@code executeTestScenario} dans
 * son propre {@link #execute()}.
 *
 * <p>Constructeur recommandé : 5 arguments {@code (reflectionInboundPortURI, scenario, nbThreads,
 * nbSchedulableThreads, brokerReflectionURI)}. Le constructeur 4-arg est conservé pour rétro-compat
...
public class ScenarioPluginClient extends PluginClient {
	...
	/**
	 * Construit un participant de scénario relié à un broker connu par son URI de réflexion.
	 *
	 * @param reflectionInboundPortURI URI du port de réflexion (= URI participant dans le
	 *     scénario).
	 * @param scenario scénario à exécuter.
	...
	/**
	 * Initialise l'horloge accélérée puis lance l'exécution du scénario si ce composant y apparaît
	 * comme participant.
	 *
	...
	public void execute() throws Exception {
		...
		if (this.scenario != null
				&& this.scenario.entityAppearsIn(this.getReflectionInboundPortURI())) {
	...
	/**
	 * Surcharge du hook {@code onReceive} de {@link PluginClient} qui ajoute une trace console
	 * préfixée {@code [MidSemScenario][RECEIVE]} pour rendre les livraisons visibles pendant la
	 * démo.
	 *
