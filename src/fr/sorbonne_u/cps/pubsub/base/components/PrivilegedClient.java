...
/**
 * Composant client de démonstration jouant le rôle de <strong>privilégié</strong> : publie ET gère
 * ses propres canaux privilégiés (création / destruction / modification d'autorisations). Compose
 * deux plugins :
 *
 * <ul>
 *   <li>{@link ClientRegistrationPlugin} — registration + port {@code ReceivingCI} ;
 *   <li>{@link ClientPrivilegedPlugin} — port sortant {@link
 *       fr.sorbonne_u.cps.pubsub.interfaces.PrivilegedClientCI} (qui inclut publication ; cf. CDC
 *       §3.6).
 * </ul>
 *
 * <p><strong>Convention constructeur</strong> : {@code (uri, brokerReflectionURI, scenario,
 * registrationClass)}. La classe initiale est généralement FREE puis upgradée à STANDARD ou PREMIUM
 * via {@link #modifyServiceClass} avant d'invoquer {@link #createChannel}.
 *
...
public class PrivilegedClient extends AbstractComponent {
	...
	/**
	 * Crée un canal privilégié dont l'accès est restreint aux URIs de port de réception
	 * correspondant au regex {@code authorisedUsers}.
	 *
	 * @param channel nom du canal à créer (arbitraire, hors patron {@code channel\d+}).
	 * @param authorisedUsers regex appliqué sur les URIs de port de réception des clients ; cf. CDC
	 *     §3.6.
	...
	/**
	 * Modifie la liste (regex) des utilisateurs autorisés sur un canal privilégié déjà créé par ce
	 * client.
	 *
	...
	public void modifyAuthorisedUsers(String channel, String authorisedUsers)
			throws UnknownClientException, UnauthorisedClientException, UnknownChannelException {
