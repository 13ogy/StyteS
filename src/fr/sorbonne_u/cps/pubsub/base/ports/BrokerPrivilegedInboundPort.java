...
/**
 * Port inbound exposant les opérations de gestion des canaux privilégiés du broker via l'interface
 * {@link PrivilegedClientCI}.
 *
 * <p><strong>Propriétaire</strong> : {@link Broker}.
 *
 * <p>Comme {@link PrivilegedClientCI} étend {@link
 * fr.sorbonne_u.cps.pubsub.interfaces.PublishingCI}, ce port hérite également des opérations {@code
 * publish} / {@code asyncPublishAndNotify} via {@link BrokerPublishingInboundPort}.
 *
 * <p>Convention : les exceptions métier déclarées sur la CI sont propagées telles quelles ; toute
 * autre {@link Exception} technique est encapsulée dans une {@link RemoteException}. : les
 * opérations void véritablement fire-and-forget ({@link #modifyAuthorisedUsers}, {@link
 * #destroyChannel}) sont dispatchées sur l'executor de réception ; leurs exceptions sont logguées
 * (l'appelant ne peut pas les observer). Les opérations dont la sémantique synchrone importe à
 * l'appelant ({@link #createChannel} : remontée de quota / unicité, {@link #destroyChannelNow} :
 * disparition garantie au retour) restent basées sur {@link
 * fr.sorbonne_u.components.AbstractComponent#handleRequest handleRequest}.
 *
 * @author Bogdan Styn, Setbel Mélissa
 */
public class BrokerPrivilegedInboundPort extends BrokerPublishingInboundPort
	...
	/** Constructeur à URI explicite — le broker dérive cette URI depuis son URI de réflexion. */
	...
	public boolean hasCreatedChannel(String receptionPortURI, String channel) throws Exception {
		try {
			return this.getOwner()
					.handleRequest(o -> ((Broker) o).hasCreatedChannel(receptionPortURI, channel));
	...
	public boolean channelQuotaReached(String receptionPortURI) throws Exception {
		try {
			return this.getOwner()
					.handleRequest(o -> ((Broker) o).channelQuotaReached(receptionPortURI));
		} catch (UnknownClientException e) {
			throw e;
		} catch (Exception e) {
			throw new RemoteException(e.getMessage(), e);
	...
	/**
	 * Création de canal privilégié — synchrone car l'appelant a besoin de la réponse métier (quota
	 * / unicité) immédiatement.
	 *
						...
						try {
							((Broker) o)
									.modifyAuthorisedUsers(
											receptionPortURI, channel, autorisedUsers);
						} catch (Exception e) {
	...
	/**
	 * Destruction asynchrone — le canal devient invisible mais son nettoyage profond se fait après
	 * vidage du compteur in-flight.
	 *
	...
	/**
	 * Destruction synchrone — l'appelant attend la suppression effective de tout l'état du canal.
	 *
