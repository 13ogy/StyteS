...
/**
 * Port inbound exposant le service de publication du broker via l'interface {@link PublishingCI}.
 *
 * <p><strong>Propriétaire</strong> : {@link Broker} (cast effectué dans chaque méthode pour
 * récupérer le composant concret).
 *
 * <p>chaque méthode void route l'appel broker effectif via {@link
 * Broker#getReceptionExecutorIndex()} dans {@link
 * fr.sorbonne_u.components.AbstractComponent#runTask(int,
 * fr.sorbonne_u.components.ComponentI.ComponentTask)}, de sorte que la thread RMI rend la main
 * immédiatement. Toute exception levée dans la lambda (y compris les violations de précondition CDC
 * §C.2) est logguée sur le tracer du broker plutôt que propagée — la publication est
 * fire-and-forget (CDC §3.5).
 *
 * <p>la convention du projet est respectée — les exceptions métier déclarées sur la CI sont
 * propagées telles quelles ; toute autre {@link Exception} technique est encapsulée dans une {@link
 * RemoteException}.
 *
...
public class BrokerPublishingInboundPort extends AbstractInboundPort implements PublishingCI {
	...
	/**
	 * Constructeur exposant une CI plus spécifique (utilisé par les sous-classes comme {@link
	 * BrokerPrivilegedInboundPort}).
	 */
	...
	/**
	 * Constructeur à URI explicite : permet au broker de choisir une URI déterministe dérivée de
	 * son URI de réflexion.
	 */
	...
	/**
	 * Constructeur à URI explicite + interface utilisé par les sous-classes (par exemple {@link
	 * BrokerPrivilegedInboundPort}) qui souhaitent une URI déterministe tout en déclarant une CI
	 * plus spécifique.
	 */
